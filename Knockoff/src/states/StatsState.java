...
public class StatsState extends State {
	...
	public void render(Graphics g) {
		...
		g.drawString("Total Downs", handler.getWidth() / 4 - 160, yStart + 100);
		g.drawString("Perks Ate", handler.getWidth() / 4 - 160, yStart + 150);
		g.drawString("Perk Spins", handler.getWidth() / 4 - 160, yStart + 200);
		g.drawString("Box Pulls", handler.getWidth() / 4 - 160, yStart + 250);
		g.drawString("Box Spins", handler.getWidth() / 4 - 160, yStart + 300);
		g.drawString("Traps Used", handler.getWidth() / 4 - 160, yStart + 350);
		g.drawString("K/D Ratio", handler.getWidth() / 4 - 160, yStart + 400);
		g.drawString("Average Round", handler.getWidth() / 4 - 160, yStart + 450);

		g.drawString(Long.toString(handler.getGlobalStats().getTotalGames()), handler.getWidth() / 2 - 160, yStart);
		...
		g.drawString(Long.toString(handler.getGlobalStats().getGlobalDowns()), handler.getWidth() / 2 - 160,
				yStart + 100);
		g.drawString(Long.toString(handler.getGlobalStats().getPerksAte()), handler.getWidth() / 2 - 160, yStart + 150);
		g.drawString(Long.toString(handler.getGlobalStats().getPerkSpins()), handler.getWidth() / 2 - 160,
				yStart + 200);
		g.drawString(Long.toString(handler.getGlobalStats().getBoxPulls()), handler.getWidth() / 2 - 160, yStart + 250);
		g.drawString(Long.toString(handler.getGlobalStats().getBoxSpins()), handler.getWidth() / 2 - 160, yStart + 300);
		...
		if (handler.getGlobalStats().getGlobalDowns() <= 0) {
			g.drawString(df.format(
					...
					handler.getWidth() / 2 - 160, yStart + 400);
		} else {
			g.drawString(df.format(
