...
public class Graph {
			...
			while(temp != null){
				...
				if(u.vertex != v.vertex && queue.inQueue(v.vertex)){
					...
					if(v.dist > u.dist + temp.weight){
						...
						//System.out.println("Relaxed: " +  vertices[v.vertex].vertex + " pred is " + vertices[v.vertex].pred);
					}
	...
	//the find command with the flag value of 0
//	public int findPath(int src, int dest){
//		//find the shortest path, which is stored in the data of the vertices array
//		//inside of the the graph data structure
//		dijkstra(vertices[src].vertex);
//
//		//if the distance to the destination vertex is INT_MAX, or a overflowed
//		//negative value, that means there is no path from the source to destination
//		if(vertices[dest].dist < 0 || vertices[dest].dist == 2147483647){
//			return -1;
//		}
//		else if(src == dest && vertices[dest].pred == src) {
//			return src;
//		}
//		//otherwise print the path
//		else{
...
//			//using the vertex predecessor value, we could traverse back from the
//			//destination vertex to the source vertex, giving us our path backwards
//			Vertex traverse = vertices[dest];
//			//path.add(traverse.vertex);
//			//traverse until we hit the source vertex
//			while(traverse.vertex != src){
//				//"save" our path
//				if(traverse.pred != src)
//					nextStep = traverse.pred;
//				//path.add(traverse.pred);
//				//go to next predecessor
//				if(traverse.pred != -1)
//					traverse = vertices[traverse.pred];
